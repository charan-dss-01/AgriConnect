...
export default function MyProducts() {
  ...
  useEffect(() => {
    const fetchMyProducts = async () => {
      ...
      try {
        const { data } = await axios.get(`${API_BASE_URL}/api/products/my-products`, { withCredentials: true });
        console.log(data);
        setMyProducts(data);
      } catch (error) {
        console.log(error);
  ...
  const handleDelete = async (id) => {
    await axios.delete(`${API_BASE_URL}/api/products/delete/${id}`, { withCredentials: true })
      .then((res) => {
